...
public class ClientHandler {
  ...
  private void handleTransactionalStartReq(ClientMessageReq clientReq) {
    ...
    clientStateLock.unlock();
    int type = 0;
    if (clientReq.hasToType()) {
      type = clientReq.getType();
    }
    final int finalType = type;
    int appId = 0;
    if (clientReq.hasToAppId()) { // TBU
      appId = clientReq.getAppId();
    }
    final int finalAppId = appId;
    proxy.executeAsync(() -> trxManager.startTransaction(this.clientId, t, finalType, finalAppId),
  ...
  private void handleTransactionalOpsReq(ClientMessageReq clientReq, Transaction trx) {
    // System.out.println("handleTransactionalOpsReq");
  ...
  public void onTransactionStarted(Transaction t, boolean success) {
    ...
    if (t.getTrxState() != TxState.ABORTED) {
      if (success) {
        ...
        proxy.executeAsync(() -> trxManager.startTransaction(this.clientId, t, t.cluster, t.appId),
        proxy.getDefaultExpHandler());
