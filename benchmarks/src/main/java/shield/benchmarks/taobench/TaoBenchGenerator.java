...
public class TaoBenchGenerator {
    ...
    public void runNextTransaction() {
        ...
        if (x < config.PROB_TRX_GROUP) {
            trx = GenerateGroupInput(config.TXN_SIZE_GROUP);
...
//            System.out.println("[" + TaoBenchConstants.Transactions.READ + "] End");
        } else if (x < config.PROB_TRX_READ) {
...
//            System.out.println("[" + TaoBenchConstants.Transactions.WRITETRANSACTION + "] End");
    ...
    public GroupTransaction GenerateGroupInput(Integer txnSize) {
        return new GroupTransaction(this,txnSize, txn_id++);
    }
