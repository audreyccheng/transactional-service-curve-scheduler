...
public final class Client extends ClientBase {
  ...
  public void scheduleTransaction(int type) throws DatabaseAbortException {
    clientTransaction.setType(type);
    // System.out.println("Schedule cluster: " + type);
  }

  public void scheduleTransactionFair(int type, int appId) throws DatabaseAbortException {
    clientTransaction.setType(type);
    clientTransaction.setAppId(appId);
    // System.out.println("Schedule cluster: " + type);
  }
  ...
  public synchronized void executeOps() throws DatabaseAbortException {
    ...
    if (clientTransaction.isFirstRequest()) {
      ...
      int type = clientTransaction.getType();
      if (type != 0) {
        bClientMsg.setToType(true);
        bClientMsg.setType(type);
      }

      int appId = clientTransaction.getAppId();
      if (appId != 0) {
        bClientMsg.setToAppId(true); // TBU
        bClientMsg.setAppId(appId);
      }
    }
    ...
    try {
      ...
      // logOut("Sent msg ", Logging.Level.FINE);
    } catch (Exception e) {
